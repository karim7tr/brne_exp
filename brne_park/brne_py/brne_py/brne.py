...
def brne_nav(xtraj_samples, ytraj_samples, num_agents, tsteps, num_pts, 
             cost_a1, cost_a2, cost_a3, ped_sample_scale, y_min, y_max,
             tree_x=4.85646, tree_y=0.0368093, tree_radius=0.5, tree_repulsion_gain=8.0,
             wall_avoidance_threshold=0.2, wall_penalty_gain=8.0,
             fixed_ped_x=None, fixed_ped_y=None, fixed_ped_radius=0.5, fixed_ped_penalty_gain=15.0):
    # Create index table and initialize weights as before.
    ...
    all_costs = costs_nb(xtraj_samples, ytraj_samples, num_agents, num_pts, tsteps,
                         cost_a1, cost_a2, cost_a3)
    # Use a collision mask that checks if each sample is within [y_min, y_max].
    ...
    for i in range(num_pts):
        ...
        for t in range(tsteps):
            ...
            if y < y_min or y > y_max:
                # If the sample goes out-of-bounds, add a very large penalty.
                total_penalty += 100.0
            else:
                wall_dist = min(y - y_min, y_max - y)
                if wall_dist < wall_avoidance_threshold:
                    # Square the factor to steepen the penalty.
                    total_penalty += wall_penalty_gain * ((1.0 - wall_dist / wall_avoidance_threshold) ** 2)
            
            # Tree penalty:
            dx_tree = x - tree_x
            dy_tree = y - tree_y
            tree_dist = np.hypot(dx_tree, dy_tree)
            if tree_dist < tree_radius:
                total_penalty += tree_repulsion_gain * ((1.0 - tree_dist / tree_radius) ** 2)
            
            # Fixed pedestrian penalty:
            if fixed_ped_x is not None and fixed_ped_y is not None:
                for j in range(len(fixed_ped_x)):
                    dx_fixed = x - fixed_ped_x[j]
                    dy_fixed = y - fixed_ped_y[j]
                    fixed_dist = np.hypot(dx_fixed, dy_fixed)
                    if fixed_dist < fixed_ped_radius:
                        total_penalty += fixed_ped_penalty_gain * ((1.0 - fixed_dist / fixed_ped_radius) ** 2)
        # Multiply by an extra factor (here 2.0) so that unsafe samples have much lower weight.
        weights[0, i] *= np.exp(-2.0 * total_penalty)
    # ---------------------------------------------------------------------------
