...
public:
  PathPlan()
  : Node("brne"), goal_set{false}, walls_generated{false}, trial_used_brne(false)
  {
    // --- CSV Logging for step data ---
    // We fix the CSV filename; here, the file is overwritten whenever a goal is received.
    step_csv_filename = "output_data_park.csv";

    // --- Parameter declarations ---
    declare_parameter("replan_freq", 0.5);
    ...
    declare_parameter("wall_avoidance_threshold", 0.5);  // meters
    declare_parameter("wall_penalty_gain", 5.0);
    declare_parameter("offset_unitree_vel", false);
    declare_parameter("open_space_velocity", 0.5);
...
private:
  ...
  double replan_freq, kernel_a1, kernel_a2, cost_a1, cost_a2, cost_a3, y_min, y_max, dt,
         ...
         nominal_lin_vel, close_stop_threshold, wall_avoidance_threshold, wall_penalty_gain;
  ...
  // CSV-related members for step data
  ...
  void goal_cb(const geometry_msgs::msg::PoseStamped & msg)
  {
    ...
    // Write CSV header for step data (this overwrites the file)
    {
  ...
  void pedestrians_cb(const crowd_nav_interfaces::msg::PedestrianArray & msg)
  {
    ...
    const auto current_time_sec = current_timestamp.seconds();

    for (auto p : ped_buffer.pedestrians) {
      auto ped_time_sec = rclcpp::Time(p.header.stamp).seconds();
      auto dt = current_time_sec - ped_time_sec;
      if (dt > people_timeout) continue;

      // Remove the speed check to include static pedestrians
  ...
  void timer_callback()
  {
    ...
    if (n_agents > 1) {
      ...
      // When no pedestrians are detected, use open_space_velocity.
      const auto opt_cmds = trajgen.opt_controls(goal_vec);
      if (goal_set) {
        for (int i = 0; i < n_steps; i++) {
          geometry_msgs::msg::Twist tw;
          // Override the linear velocity with open_space_velocity.
          tw.linear.x = get_parameter("open_space_velocity").as_double();
          // Use the computed angular command.
  ...
  void pub_walls()
  {
    if (!walls_generated) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        wall.pose.position.x = WALL_LENGTH / 2;
        wall.pose.position.y = (i == 0) ? -WALL_LENGTH / 2 : WALL_LENGTH / 2;
      ...
      for (int i = 2; i < 4; i++) {
        ...
        wall.type = visualization_msgs::msg::Marker::CUBE;
        wall.action = visualization_msgs::msg::Marker::ADD;
        wall.pose.position.x = (i == 2) ? 0 : WALL_LENGTH;
    ...
    for (auto & wall : wall_markers.markers) {
