...
private:
  ...
  void pedestrians_cb(const crowd_nav_interfaces::msg::PedestrianArray & msg)
  {
    ...
    for (int i = 0; i < n_pedestrians; i++) {
      ...
      // Example change: use orange spheres for pedestrians
      visualization_msgs::msg::Marker marker;
      marker.header.stamp = current_time;
      marker.header.frame_id = "base_link";  // or your preferred frame
      marker.ns = "pedestrians";
      marker.id = i;  // i is the pedestrian index
      ...
      marker.color.g = 0.4;
      marker.color.b = 0.0;
      marker.color.a = alpha;  // from existing code
      marker.lifetime = rclcpp::Duration(0, 500000000); // half second

      marker.pose.position.x = ped.pose.position.x;
      marker.pose.position.y = ped.pose.position.y;
      marker.pose.position.z = 0.0;  // small height
